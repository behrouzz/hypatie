...
def equations(T):
    """
    Get equations of equinoxes and origins and C matrix
    ...
    Return:
    -------
        equation_of_equinoxes (arcseconds)
        equation_of_origins (arcseconds)
        C matrix
    """
    N, other = nutation(T)
    F, D, om, d_psi, e = other
    P = precession(T)
    ...
    equation_of_equinoxes = d_psi * np.cos(e) \
    ...
    # Equation of origins (arcsec)
    equation_of_origins = - (par + equation_of_equinoxes)
    eq_o_rad = equation_of_origins * s2r

    # position of CIO in GCRS
    sig_GCRS = eq_GCRS * np.cos(eq_o_rad) - np.cross(n_GCRS, eq_GCRS) * np.sin(eq_o_rad)

    # page 65
    y_GCRS = np.cross(n_GCRS, sig_GCRS)

    C = np.array([sig_GCRS, y_GCRS, n_GCRS])
    # page 66 and 48 (MOHEM: combined method)
    #r_CIRS = C * r_GCRS
    return equation_of_equinoxes, equation_of_origins, C
